...
PROJECT_CUTOFF = datetime(
    2026, 9, 12, 23, 59, 59, tzinfo=timezone.utc
)
FULL_FUNNEL = [
    "ad_click",
    "landing_view",
...
def test_events_are_chronological_inside_each_scenario():
    ...
    for event in read_csv(EVENTS_PATH):
        by_scenario[event["scenario_id"]].append(
...
def test_control_dates_are_not_later_than_project_cutoff():
    ...
    events = read_csv(EVENTS_PATH)
    assert all(
        parse_utc(row["published_at"]) <= PROJECT_CUTOFF
        for row in placements
    )
    assert all(
        parse_utc(row["occurred_at"]) <= PROJECT_CUTOFF
        for row in events
    )
...
def test_stage_identifiers_and_relationships_are_present():
    ...
    leads = {
        e["parsed_properties"]["lead_id"]: e
        for e in events
        if e["event_name"] == "lead_created"
    }
    orders = {
        e["parsed_properties"]["order_id"]: e
        for e in events
    ...
    for event in events:
        ...
        if name in LEAD_STAGE_EVENTS:
            assert properties.get("lead_id") in leads
        if name in ORDER_STAGE_EVENTS:
            assert properties.get("order_id") in orders
        if name in PAYMENT_STAGE_EVENTS:
            assert properties.get("payment_id")
        if name == "order_created":
            lead = leads[properties["lead_id"]]
            assert lead["visitor_id"] == event["visitor_id"]
        if name == "payment_succeeded":
            order = orders[properties["order_id"]]
            assert order["parsed_properties"]["lead_id"] == (
                properties["lead_id"]
            )
...
def test_successful_payments_and_expected_revenue():
    payments = [
        e for e in parsed_events() if e["event_name"] == "payment_succeeded"
    ]
    ...
    for payment in payments:
        ...
        assert parse_utc(properties["paid_at"]) == parse_utc(
            payment["occurred_at"]
        )
...
def test_expected_control_counts():
    ...
    assert (
        sum(row["event_name"] == "payment_succeeded" for row in events) == 2
    )
...
def test_scenario_02_encodes_expected_last_touch_fixture():
    ...
    lead_occurred_at = parse_utc(lead["occurred_at"])
    eligible = []
    for click in clicks:
        delta = lead_occurred_at - parse_utc(click["occurred_at"])
        if timedelta(0) <= delta <= timedelta(days=30):
            eligible.append(click)
    assert max(eligible, key=lambda row: row["occurred_at"])[
        "placement_id"
    ] == "pl_005"
